...
![](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Math__StochasticProcess__assets__02-Markov.assets__20250428103145-20250715171105400.webp)
...
![](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Math__StochasticProcess__assets__02-Markov.assets__20250517192308.webp)
...
例题1
![](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Math__StochasticProcess__assets__02-Markov.assets__202506091146554.webp)

例题2
![](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Math__StochasticProcess__assets__02-Markov.assets__202506091148965.webp)

例题3
![](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Math__StochasticProcess__assets__02-Markov.assets__202506091149496.webp)
![](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Math__StochasticProcess__assets__02-Markov.assets__202506091149867.webp)
...
1. (伯努利-拉普拉斯扩散模型)设 A, B 两箱中各有 $m$ 个球，其中共 $m$ 个白球$,m$ 个黑球. 记 $X_0$ 是开始时 A 箱中的白球个数.然后每次从两箱中各任取一球交换，$X_n$表示$n$次交换后 A 箱中白球个数说明$\{X_n\}$是一个时齐马尔可夫链，写出状态空间和一步转移概率.这也是一个关于两种液体混合的概率模型.
...
$$
\therefore Y_n = \begin{cases} 
1 \\
0 \\
-1 
\end{cases} \quad \text{独立同分布}
$$
...
$$
P_{ij} = \begin{cases} 
\left(\frac{m-i}{m}\right)^2, & j = i+1, i \neq m \\
\left(\frac{i}{m}\right)^2, & j = i-1, i \neq 0 \\
\frac{2i(m-i)}{m^2}, & j = i \\
0, & \text{其它}
\end{cases}
$$
...
$$
L_{n}= \begin{cases}0, & X_{n}=0, \\ \max \{1 \leqslant k \leqslant n ; X_{n} X_{n-1} \cdots X_{n-k+1}=1\}, & X_{n}=1\end{cases}
$$
...
(1) 计算 $P(Y_{2} = 1 \mid Y_{0} = 1, Y_{1} = 6)$, $P(Y_{2} = 1 \mid Y_{1} = 6)$;
(2) 计算 $P(Z_{2} = 12 \mid Z_{0} = 2, Z_{1} = 7)$, $P(Z_{2} = 12 \mid Z_{1} = 7)$;
(3) 判断 $\{Y_{n}\}$ 和 $\{Z_{n}\}$ 是否具有马尔可夫性? 说明理由.
...
Y_0 = \max\{X_1, X_2\} = 1 \\
Y_1 = \max\{X_2, X_3\} = 6 \\
\Rightarrow X_2 \geq 1 \quad \text{且} \quad X_3 = 6
$$

因此
$$
Y_2 = \max\{X_3, X_4\} \geq 6
$$

$$
\Rightarrow P(Y_2 = 1 \mid Y_0 = 1, Y_1 = 6) = 0
$$
...
Y_1 = \max\{X_2, X_3\} = 6  \\
Y_2 = \max\{X_3, X_4\} = 1 \\
\Rightarrow X_2 = 6 \text{ 且 } X_3 = 1 \text{ 且 } X_4 = 1
$$

$$
\Rightarrow P(Y_2 = 1 \mid Y_1 = 6) = \frac{P(Y_2 = 1, Y_1 = 6)}{P(Y_1 = 6)} = \frac{\frac{1}{6} \times \frac{1}{6} \times \frac{1}{6}}{\frac{11}{36}} = \frac{1}{66}
$$

---

$$
Z_0 = 2 = X_1 + X_2 \Rightarrow X_1 = X_2 = 1
$$

$$
Z_1 = X_2 + X_3 = 7 \Rightarrow X_3 = 6
$$

$$
Z_2 = 12 \Rightarrow X_4 = 6 = X_3
$$

$$
\Rightarrow P(Z_2 = 12 \mid Z_0 = 2, Z_1 = 7) = \frac{1}{6}
$$

计算 $P(Z_2 = 12 \mid Z_1 = 7)$:

$$
P(Z_2 = 12 \mid Z_1 = 7) = \frac{P(Z_2 = 12, Z_1 = 7)}{P(Z_1 = 7)} \\
= \frac{\frac{1}{6} \times \frac{1}{6} \times \frac{1}{6}}{\frac{1}{36}} = \frac{1}{36}
$$

---

$$
P(Y_2 = 1 \mid Y_0 = 1, Y_1 = 6) \neq P(Y_2 = 1 \mid Y_1 = 6)
$$

$$
P(Z_2 = 12 \mid Z_0 = 2, Z_1 = 7) \neq P(Z_2 = 12 \mid Z_1 = 7)
$$

$$
\Rightarrow\{Y_n\}, \{Z_n\} \text{ 不具有马尔可夫性}
$$

### 3.7

7. 在单位圆上等距取3个点，按顺时针方向记为0,1,2。当一质点位于状态 $i (i = 0, 1, 2)$ 时，下一时刻以 $\frac{2}{3}$ 概率顺时针走一格，以 $\frac{1}{3}$ 概率逆时针走一格。以 $X_0$ 表示初始时刻的位置，设 $P(X_0 = 0) = \frac{1}{2}$，$P(X_0 = 1) = P(X_0 = 2) = \frac{1}{4}$。令 $X_n$ 表示 $n$ 时刻质点所处的位置，则 $\{ X_n; n = 0, 1, \cdots \}$ 是一时齐马尔可夫链。
(1) 计算一步转移矩阵；
(2) 计算 $P(X_0 = 0, X_2 = 0, X_4 = 1)$ 和 $P(X_2 = 1)$。
...
$$
P(X_0 = 0, X_2 = 0, X_4 = 1) = P(X_0 = 0) \cdot P_{00}^{(2)} \cdot P_{01}^{(2)} \\
= \frac{1}{2} \times \frac{4}{9} \times \frac{1}{9} = \frac{2}{81}
$$

$$
P(X_2 = 1) = \sum_{i=0}^{2} P(X_0 = i) \cdot P_{i1}^{(2)} \\
= \frac{5}{18}
$$
...
设 $P(X_0 = 0) = P(X_0 = 1) = P(X_0 = 2) = \frac{1}{3}$.
(1) 计算 $P(X_2 = 0 | X_0 = 0)$ 和 $P(X_0 = 0 | X_2 = 0)$;
(2) 计算 $P(X_1 = 0)$ 和 $P(X_1 = 0, X_3 = 0, X_4 = 1, X_6 = 1)$;
(3) 计算 $f_{11}^{(n)}$, $f_{11}$ 和 $\mu_1$
...
$$
\text{两步转移概率矩阵}~P^2 = \begin{bmatrix}
...
$$
\begin{aligned}
P(X_2 = 0 \mid X_0 = 0) &= P_{00}^{(2)} = \frac{2}{9} \\[1em]
P(X_0 = 0 \mid X_2 = 0) &= \frac{P(X_0 = 0, X_2 = 0)}{P(X_2 = 0)} = \frac{2}{5} \\[1em]
P(X_1 = 0) &= \sum_{i=0}^{2} P(X_0 = i) P_{i0}^{(1)} \\
&= \frac{1}{3} \left( \frac{2}{3} + \frac{1}{3} + 0 \right) = \frac{1}{3} \\[1em]
P(X_1=0,X_3 = 0, X_4 = 1,X_6=1) &= P(X_1 = 0) P_{00}^{(2)} P_{01}^{(1)} P_{11}^{(2)} \\
&= \frac{1}{3} \times \frac{5}{9} \times \frac{2}{3} \times \frac{7}{9} = \frac{70}{729}
\end{aligned}
$$
...
$$
f_{11}^{(2)} = \frac{2}{3} \times \frac{2}{3} + \frac{1}{3} \times 1 = \frac{7}{9}
$$


$$
f_{11}^{(n)} = 4(\frac{1}{3})^n, n \geq 3
$$
...
$$
\mu_1 = \sum n\cdot f_{11}^{(n)} = \frac{7}{3}
$$
...
![](https://philfan-pic.oss-cn-beijing.aliyuncs.com/web_pic/Math__StochasticProcess__assets__02-Markov.assets__image-20250428202251387.webp)
...
$$
\begin{aligned}
h_1 &= \frac{1}{2} h_2+\frac{1}{2}h_4 \\
h_2 &= \frac{1}{3} h_1+\frac{1}{3}h_3 + \frac{1}{3}h_5 \\
h_3 &= \frac{1}{2} h_2+\frac{1}{2}h_6 \\
h_4 &= \frac{1}{3} h_1+\frac{1}{3}h_5 + \frac{1}{3} \\
h_5 &= \frac{1}{4} h_2+\frac{1}{4}h_4 + \frac{1}{4}h_6 + \frac{1}{4}h_8 \\
h_6 &= \frac{1}{3} h_3+\frac{1}{3}h_5 \\
h_8 &= \frac{1}{3} h_5+\frac{1}{3}
\end{aligned}
$$
...
(1) 说明 $\{Z_n\}$ 是一个时齐马尔可夫链，写出状态空间和一步转移矩阵；
(2) 计算蜘蛛在位置 0 吃掉苍蝇的概率；
(3) 求蜘蛛遇见苍蝇的平均步数.

解：

1. 状态空间$I = \{(0,0),(0,1),(1,0),(1,1)\}$
   ...
   可以发现$P(X_{n+1} = j \mid X_n = i) \stackrel{\text{记为}}{=} p_{ij}$不依赖于$n$，因此其为时齐Markov

2. 令$h_{ij}$代表$Z_n$从$(i,j)$出发在有限时间到达$(0,0)$的概率，则有$h_{00} =1 ,h_{11}=0$

   根据一步转移矩阵有如下公式：

   $$
   h_{01} = 0.1+0.4h_{01}+0.1h_{10} \\
   h_{10} = 0.2+0.3h_{01}+0.2h_{10} \\
   $$

   求解上述公式可得$h_{01} = \frac{2}{9},h_{10} = \frac{1}{3}$

   因此蜘蛛在位置0吃掉苍蝇的概率为$\frac{2}{9}$

3. 我们发现$(0,1),(1,0)$是暂留态，集合C$\{(0,0),(1,1)\}$为吸收态，我们令$a_{ij} = E(T_C| Z_{ij}=(i,j))$表示从$(i,j)$出发进入C的平均步数，则有
...
$$
P(X_1 = 4,X_3 = 2) = p_{14}^{(1)}p_{42}^{(2)} = \frac{1}{18}
$$

$$
P(X_2 = 1| X_3 = 3) = \frac{P(X_2 = 1, X_3 = 3)}{P(X_3 = 3)} = \frac{p_{11}^{(2)}p_{13}^{(1)}}{\frac{7}{9}} = \frac{1}{7}
$$

---

狼恰好在第n步吃掉羊的概率，即为$f_{12}^{(n)}$

$$
f_{12}^{(n)}= \begin{cases}
(\frac{1}{6})^k\cdot \frac{1}{3} & n = 2k+1,k\ge 0\\
0 & n = 2k,k\ge 0
\end{cases}
$$
...
我们发现$\{1,2,4\}$是暂留态，集合C$\{3\}$为吸收态，我们令$a_{i} = E(T_C| Z_{i}=(i))$表示从$i$出发进入C的平均步数，则有$a_{3}=0$

$$
a_i = 1+\sum_{j} p_{ij}a_j
$$

$$
\begin{align*}
a_{1} &= \frac{1}{3}a_{2}+\frac{1}{3}a_{4} +1\\
a_{2} &= \frac{1}{2}a_{1} +1\\
a_{4} &= \frac{1}{2}a_{1}+1 
\end{align*}
$$
