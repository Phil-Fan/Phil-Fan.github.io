...
就是启动电流，目的是为了选择熔断器、镇定低压断路器

- 单台： $I_{pk} = I_{st}= K_{kt}I_{N}$,$I_N$ 额定电流，$K_{st}$启动电流倍数
- 多台

\textbf{2、给多台用电设备供电的干线尖峰电流计算}

计算公式为：
\[
I_{\text{pk}} = K_{\Sigma} \sum_{i=1}^{n-1} I_{Ni} + I_{\text{stmax}}
\]
或
\[
I_{\text{pk}} = I_c + (I_{\text{st}} - I_{\text{N}})_{\text{max}}
\]

式中：

- \( I_{\text{stmax}} \) 为用电设备组中起动电流与额定电流之差为最大的那台设备的起动电流
- \( (I_{\text{st}} - I_{\text{N}})_{\text{max}} \) 为用电设备组中起动电流与额定电流之差为最大的那台设备的起动电流与额定电流之差
- \( K_{\Sigma} \) 为上述 \( n-1 \) 台设备的同时系数，其值按台数多少选取，一般为 \( 0.7 \sim 1 \)
